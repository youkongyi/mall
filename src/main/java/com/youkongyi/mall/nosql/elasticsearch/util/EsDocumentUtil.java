...
public class EsDocumentUtil {
     ...
     * @throws Exception
     */
    public static List<BulkResponseItem> delDocByIds(String indexName, List<String> documentId) throws Exception {
        ElasticsearchClient client = ESClientPool.getClient();
        ...
        for (int i = 0; i < documentId.size(); i++) {
            int finalI = i;
            bulkOperations.add(BulkOperation.of(b -> b.delete((d -> d.index(indexName).id(documentId.get(finalI))))));
        }
